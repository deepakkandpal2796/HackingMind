...
//# This is better than the quer selector create a hashmap of all the ids on the page with corresponding refrences 
...
function incrementCounter (){
    ...
    // console.log(li)
    const b = document.createElement('b') //! b tag is bold in html
...
//! hume bold me text node chahiye tha, and then both the textnode in li 
...
function decrementCounter (){
    const li = ulElement.querySelector('[data-counter = "'+counter+'"]'); //! to select the data-* attribute we have to write syntax like this, this will select the latest li element.
    //we can also get the attribute just like we are setting the arrtibute to a node 
    ...
    if(number % 2 === 0){ //! here we are cheching the attribute of li is what 
